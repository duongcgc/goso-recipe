<?php
add_action( 'goso_amp_post_template_head', 'goso_recipe_json_schema' );
add_action( 'wp_head', 'goso_recipe_json_schema' );

if( ! function_exists( 'goso_recipe_json_schema' ) ){
	function goso_recipe_json_schema(){
		global $post;
		if ( is_singular() ) {
			...
			if( $recipe_title ) {
				...
				$recipe_totaltime_fm  = get_post_meta( $recipe_id, 'goso_recipe_totaltime_format', true ) ? get_post_meta( $recipe_id, 'goso_recipe_totaltime_format', true ) : $recipe_cooktime_fm;
				...
				if ( $recipe_ingredients ){
					if( ! get_theme_mod( 'goso_recipe_ingredients_visual' ) ){
						...
						$json_recipe_ingredients = goso_recipe_reverse_wpautop( $recipe_ingredients );
					}

				}

				if( $recipe_instructions ){
					$json_recipe_instructions = goso_recipe_reverse_wpautop( $recipe_instructions );
				}
				...
				$json_recipe = array(
					...
					'author'             => array(
						'@type' => 'Person',
						'name'  => goso_recipe_get_post_author(),
					),
					'description'        => $excerpt,
					'datePublished'      => get_the_date( 'Y-m-d' ),
					'recipeCategory'     => goso_recipe_get_post_category( $recipe_id ),
				...
				if( ! get_theme_mod( 'goso_recipe_remove_nutrition' ) ){
				...
				?>
				<script type="application/ld+json" class="goso-recipe-schema"><?php echo wp_json_encode( $json_recipe ); ?></script>
				<?php
			}
		}
	}
...
function goso_recipe_reverse_wpautop( $str ) {
...
function goso_recipe_get_post_author(){
	global $post;
